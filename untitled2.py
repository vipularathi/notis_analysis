...
n_tbl_test_mod = 'test_mod_2025-04-01'
# n_tbl_test_raw = n_tbl_notis_raw_data
n_tbl_test_raw = 'test_raw_2025-04-01'
# n_tbl_test_cp_noncp = n_tbl_notis_eod_net_pos_cp_noncp
n_tbl_test_cp_noncp = 'test_cp_noncp_2025-04-01'
# n_tbl_test_net_pos_desk = n_tbl_notis_desk_wise_net_position
n_tbl_test_net_pos_desk = 'test_net_pos_desk_2025-04-01'
# n_tbl_test_net_pos_nnf = n_tbl_notis_nnf_wise_net_position
n_tbl_test_net_pos_nnf = 'test_net_pos_nnf_2025-04-01'
# n_tbl_test_bse = n_tbl_bse_trade_data
n_tbl_test_bse = 'test_bse_2025-04-01'
...
def modify_file(df, df_nnf):
    ...
    # --------------------------------------------------------------------------------------------------------------------------------
    pbar = progressbar.ProgressBar(max_value=100, widgets=[progressbar.Percentage(), ' ',
    ...
    # --------------------------------------------------------------------------------------------------------------------------------
    pbar.finish()
    merged_df[['TerminalID', 'TerminalName', 'UserID', 'SubGroup', 'MainGroup', 'NeatID']] = merged_df[
        ['TerminalID', 'TerminalName', 'UserID', 'SubGroup', 'MainGroup', 'NeatID']].fillna('NONE')
...
def main(from_time, to_time):
    ...
    # main_mod_df['expDt'] = pd.to_datetime(main_mod_df['expDt'], dayfirst=True).dt.strftime('%d-%m-%Y')
    ...
    if len(main_mod_df.bsFlg.unique()) == 1:
        if main_mod_df.bsFlg.unique().tolist()[0] == 'B':
            pivot_df['SellTrdQtyPrc']=0;pivot_df['SellQty']=0
        elif main_mod_df.bsFlg.unique().tolist()[0] == 'S':
            pivot_df['BuyTrdQtyPrc']=0;pivot_df['BuyQty']=0
