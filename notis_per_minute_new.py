...
#     global main_mod_bse_df, bse_empty
#     df_bse = read_data_db(for_table='TradeHist', from_time=from_time, to_time=to_time)
#     print(f'BSE trade data fetched from {from_time.split(" ")[1]} to {to_time.split(" ")[1]}, shape={df_bse.shape}')
#     if df_bse.empty:
#         print(f'No BSE trade for {from_time.split(" ")[1]} and {to_time.split(" ")[1]} hence skipping')
#         bse_empty=True
#         return
...
#     print(f'length of main_mod_bse_df before concat is {main_mod_bse_df.shape}')
#     main_mod_bse_df = pd.concat([main_mod_bse_df,modified_bse_df],ignore_index=True)
#     print(f'length of main_mod_bse_df after concat is {main_mod_bse_df.shape}')
...
#     return eod_bse_df
#
# def get_nse_trade(from_time, to_time,bse_from_time, bse_to_time):
#     global main_mod_df, nse_empty, bse_empty
#     df_db = read_data_db(from_time=from_time,to_time=to_time)
#     print(f'Notis trade data fetched from {from_time.split(" ")[1][:-4]} to {to_time.split(" ")[1][:-4]}, shape: {df_db.shape}')
#     if df_db.empty:
#         print(f'No NSE trade between {from_time.split(" ")[1][:-4]} and {to_time.split(" ")[1][:-4]} hence skipping further processes')
#         nse_empty = True
#         a=get_bse_trade_data()
#
#     if not nse_empty and not bse_empty:
...
#         print(f'length of main_mod-df before concat is {len(main_mod_df)}')
#         main_mod_df = pd.concat([main_mod_df, modified_df], ignore_index=True)
#         print(f'length of main_mod-df after concat is {len(main_mod_df)}')
...
#     elif nse_empty and not bse_empty:
#         eod_df = read_data_db(for_table=f'NOTIS_EOD_NET_POS_CP_NONCP_{yesterday.strftime("%Y-%m-%d")}')
#         orig_eod_df = eod_df.query("EodUnderlying != 'SENSEX' and EodExpiry >= @today")
#         bse_eod_df = BSEUtility.calc_bse_eod_net_pos(eod_df)
#         new_eod_df = pd.concat([orig_eod_df,bse_eod_df], ignore_index=True)
#         write_notis_postgredb(new_eod_df, table_name=n_tbl_test_cp_noncp, truncate_required=True)
#
#
...
#                 get_nse_trade(nse_from_time,nse_to_time,bse_from_time,bse_to_time)
#                 # get_bse_trade_data(bse_from_time, bse_to_time)
#                 time.sleep(1)
#     else:
#         print(f'Today is not a business day hence exiting.')
#         exit()
p=0
import re, os, progressbar, psycopg2, pyodbc, warnings, time
import pandas as pd
...
from common import (get_date_from_non_jiffy, get_date_from_jiffy,
                    today, yesterday,
                    root_dir, logger, test_dir,
                    read_data_db, write_notis_postgredb)
from nse_utility import NSEUtility
from bse_utility import BSEUtility
...
def get_bse_trade_data(from_time, to_time):
    ...
    pivot_df.drop(columns=['BuyTrdQtyPrc', 'SellTrdQtyPrc'], inplace=True)
    pivot_df['IntradayVolume'] = pivot_df.BuyQty - pivot_df.SellQty
    pivot_df.Expiry = pd.to_datetime(pivot_df.Expiry, dayfirst=True, format='mixed').dt.date
    pivot_df = pivot_df.round(2)
    # eod_bse_df = BSEUtility.calc_bse_eod_net_pos(pivot_df)
    return pivot_df

def get_nse_trade(from_time, to_time):
    global main_mod_df, count
    ...
    pivot_df.rename(columns={'MainGroup': 'mainGroup', 'SubGroup': 'subGroup', 'sym': 'symbol', 'expDt': 'expiryDate',
                    ...
                    inplace=True)
    pivot_df.volume = pivot_df.buyAvgQty - pivot_df.sellAvgQty
    pivot_df.expiryDate = pd.to_datetime(pivot_df.expiryDate, dayfirst=True, format='mixed').dt.date
    pivot_df = pivot_df.round(2)
    desk_db_df = NSEUtility.calc_deskwise_net_pos(pivot_df)
    # write_notis_postgredb(desk_db_df, table_name=n_tbl_test_net_pos_desk, truncate_required=True)
    nnf_db_df = NSEUtility.calc_nnfwise_net_pos(pivot_df)
    ...
    # print(f'BSE eod cp noncp length: {cp_noncp_bse_df.shape}')
    ...
    # final_cp_noncp_df = pd.concat([cp_noncp_df, cp_noncp_bse_df], ignore_index=True)
    # # write_notis_postgredb(final_cp_noncp_df, table_name=n_tbl_test_cp_noncp, truncate_required=True)
    # final_cp_noncp_df.to_excel(os.path.join(test_dir,f'eod_nse_bse{count}.xlsx'),index=False)
    return pivot_df
def find_net_pos(nse_pivot_df, bse_pivot_df):
    # global eod_begin
    cp_noncp_nse_df,cp_noncp_bse_df = pd.DataFrame(), pd.DataFrame()
    # if eod_begin:
    #     eod_tablename = f'TEST_NOTIS_EOD_NET_POS_CP_NONCP_{yesterday.strftime("%Y-%m-%d")}'  # NOTIS_EOD_NET_POS_CP_NONCP_2025-03-17
    #     final_eod = read_data_db(for_table=eod_tablename)
    #     print(f'in eod begin, final_eod shape:{final_eod.shape}')
    # else:
    #     final_eod = final_eod_code
